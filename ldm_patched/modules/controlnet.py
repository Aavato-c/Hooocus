...
class ControlBase:
    def __init__(self, device=None):
        ...
        if device is None:
            device = unavoided_globals.model_management.get_torch_device()
        self.device = device
...
class ControlNet(ControlBase):
    def __init__(self, control_model, global_average_pooling=False, device=None, load_device=None, manual_cast_dtype=None):
        ...
        self.control_model_wrapped = ldm_patched.modules.model_patcher.ModelPatcher(self.control_model, load_device=load_device, offload_device=unavoided_globals.model_management.unet_offload_device())
...
class ControlLora(ControlNet):
    ...
    def pre_run(self, model, percent_to_timestep_function):
        ...
        self.control_model.to(unavoided_globals.model_management.get_torch_device())
        ...
        for k in self.control_weights:
            if k not in {"lora_controlnet"}:
                ldm_patched.modules.utils.set_attr(self.control_model, k, self.control_weights[k].to(dtype).to(unavoided_globals.model_management.get_torch_device()))
    ...
    def inference_memory_requirements(self, dtype):
        return ldm_patched.modules.utils.calculate_parameters(self.control_weights) * unavoided_globals.model_management.dtype_size(dtype) + ControlBase.inference_memory_requirements(self, dtype)
...
def load_controlnet(ckpt_path, model=None):
    ...
    if "controlnet_cond_embedding.conv_in.weight" in controlnet_data: #diffusers format
        unet_dtype = unavoided_globals.model_management.unet_dtype()
    ...
    manual_cast_dtype = unavoided_globals.model_management.unet_manual_cast(unet_dtype, load_device)
    ...
    if pth:
        if 'difference' in controlnet_data:
            if model is not None:
                unavoided_globals.model_management.load_models_gpu([model])
