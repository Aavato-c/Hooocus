...
class BaseModel(torch.nn.Module):
    ...
    def memory_required(self, input_shape):
        if unavoided_globals.model_management.xformers_enabled() or unavoided_globals.model_management.pytorch_attention_flash_attention():
            dtype = self.get_dtype()
            if self.manual_cast_dtype is not None:
                dtype = self.manual_cast_dtype
            #TODO: this needs to be tweaked
            area = input_shape[0] * input_shape[2] * input_shape[3]
            return (area * unavoided_globals.model_management.dtype_size(dtype) / 50) * (1024 * 1024)
        else:
