        ...
        if patch_type == "diff":
            ...
            if alpha != 0.0:
                if w1.shape != weight.shape:
                    ...
                    weight += alpha * unavoided_globals.model_management.cast_to_device(w1, weight.device, weight.dtype)
        elif patch_type == "lora":
            mat1 = unavoided_globals.model_management.cast_to_device(v[0], weight.device, torch.float32)
            mat2 = unavoided_globals.model_management.cast_to_device(v[1], weight.device, torch.float32)
            if v[2] is not None:
                alpha *= v[2] / mat2.shape[0]
            if v[3] is not None:
                mat3 = unavoided_globals.model_management.cast_to_device(v[3], weight.device, torch.float32)
            ...
            w1 = unavoided_globals.model_management.cast_to_device(v[0], weight.device, torch.float32)
            w_min = unavoided_globals.model_management.cast_to_device(v[1], weight.device, torch.float32)
            w_max = unavoided_globals.model_management.cast_to_device(v[2], weight.device, torch.float32)
            ...
            if w1 is None:
                dim = w1_b.shape[0]
                w1 = torch.mm(unavoided_globals.model_management.cast_to_device(w1_a, weight.device, torch.float32),
                            unavoided_globals.model_management.cast_to_device(w1_b, weight.device, torch.float32))
            else:
                w1 = unavoided_globals.model_management.cast_to_device(w1, weight.device, torch.float32)

            if w2 is None:
                dim = w2_b.shape[0]
                if t2 is None:
                    w2 = torch.mm(unavoided_globals.model_management.cast_to_device(w2_a, weight.device, torch.float32),
                                unavoided_globals.model_management.cast_to_device(w2_b, weight.device, torch.float32))
                else:
                    w2 = torch.einsum('i j k l, j r, i p -> p r k l',
                                    unavoided_globals.model_management.cast_to_device(t2, weight.device, torch.float32),
                                    unavoided_globals.model_management.cast_to_device(w2_b, weight.device, torch.float32),
                                    unavoided_globals.model_management.cast_to_device(w2_a, weight.device, torch.float32))
            else:
                w2 = unavoided_globals.model_management.cast_to_device(w2, weight.device, torch.float32)
            ...
            if v[5] is not None:  # cp decomposition
                ...
                m1 = torch.einsum('i j k l, j r, i p -> p r k l',
                                unavoided_globals.model_management.cast_to_device(t1, weight.device, torch.float32),
                                unavoided_globals.model_management.cast_to_device(w1b, weight.device, torch.float32),
                                unavoided_globals.model_management.cast_to_device(w1a, weight.device, torch.float32))

                m2 = torch.einsum('i j k l, j r, i p -> p r k l',
                                unavoided_globals.model_management.cast_to_device(t2, weight.device, torch.float32),
                                unavoided_globals.model_management.cast_to_device(w2b, weight.device, torch.float32),
                                unavoided_globals.model_management.cast_to_device(w2a, weight.device, torch.float32))
            else:
                m1 = torch.mm(unavoided_globals.model_management.cast_to_device(w1a, weight.device, torch.float32),
                            unavoided_globals.model_management.cast_to_device(w1b, weight.device, torch.float32))
                m2 = torch.mm(unavoided_globals.model_management.cast_to_device(w2a, weight.device, torch.float32),
                            unavoided_globals.model_management.cast_to_device(w2b, weight.device, torch.float32))
            ...
            a1 = unavoided_globals.model_management.cast_to_device(v[0].flatten(start_dim=1), weight.device, torch.float32)
            a2 = unavoided_globals.model_management.cast_to_device(v[1].flatten(start_dim=1), weight.device, torch.float32)
            b1 = unavoided_globals.model_management.cast_to_device(v[2].flatten(start_dim=1), weight.device, torch.float32)
            b2 = unavoided_globals.model_management.cast_to_device(v[3].flatten(start_dim=1), weight.device, torch.float32)
...
class BrownianTreeNoiseSamplerPatched:
    ...
    def global_init(x, sigma_min, sigma_max, seed=None, transform=lambda x: x, cpu=False):
        if unavoided_globals.model_management.directml_enabled:
            cpu = True
...
def patch_all():
    if unavoided_globals.model_management.directml_enabled:
        unavoided_globals.model_management.lowvram_available = True
        unavoided_globals.model_management.OOM_EXCEPTION = Exception

    patch_all_precision()
    patch_all_clip()

    if not hasattr(unavoided_globals.model_management, 'load_models_gpu_origin'):
        unavoided_globals.model_management.load_models_gpu_origin = unavoided_globals.model_management.load_models_gpu

    unavoided_globals.model_management.load_models_gpu = patched_load_models_gpu
