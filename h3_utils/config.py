...
class DeveloperOptions(BaseModel):
    ...
    metadata_scheme: str = Field(
        METADATA_SCHEME, description="The default metadata scheme to use."
    )
    ...
    temp_path: str = Field(
        ...
        description="The default temp path to use.",
    )
    ...
    should_use_advanced: bool = Field(
        False, description="Bool: should use advanced checkbox?"
    )
    should_use_developer_debug_mode: bool = Field(
        False, description="Bool: default developer debug mode."
    )
...
class InptaintOptions(BaseModel):
    ...
    inpaint_mask_model: str = Field(
        "isnet-general-use", description="The default invert mask model to use."
    )
    inpaint_additional_prompt: Optional[str] = None
    inpaint_disable_initial_latent: bool = False
    inpaint_engine_version: str = Field(
        "v2.6", description="The default inpaint engine version to use."
    )
    inpaint_erode_or_dilate: Optional[int] = Field(None, le=64, ge=-64)
    inpaint_mask_cloth_category: str = Field(
        "full", description="The default inpaint mask cloth category to use."
    )
    inpaint_mask_sam_model: str = Field(
        "vit_b", description="The default inpaint mask sam model to use."
    )
    inpaint_method: str = Field(
        "Inpaint or Outpaint (default)",
        description="The default inpaint method to use.",
    )
    inpaint_respective_field: float = 0.618  # min 0.0 max 1.0
    use_advanced_inpaint_masking: bool = Field(
        False, description="Bool: should use advanced inpaint masking?"
    )
    inpaint_should_use_inpaint_image: bool = False
    inpaint_should_use_mask: bool = False
    inpaint_stop_ats: List[float] = Field(
        [0.5, 0.5, 0.5, 0.5], description="The default inpaint stop ats to use."
    )
...
class EnhanceMaskCtrls(BaseModel):
    ...
    enhance_prompt: str = (
        None  # Enhacement positive prompt. Uses original prompt if None
    )
    enhance_negative_prompt: str = (
        None  # Enhacement negative prompt. Uses original negative prompt if None
    )

    enhance_inpaint_mask_model: str = Field(
        "sam", description="The default enhance inpaint mask model to use."
    )
    enhance_mask_cloth_category: INPAINT_MASK_CLOTH_CATEGORY = "full"

    enhance_mask_sam_model: str = Field(
        "vit_b", description="The default inpaint mask sam model to use."
    )

    enhance_mask_text_threshold: float = Field(
        0.25,
        description="The default enhance mask text threshold to use.",
        ge=0.0,
        le=1.0,
    )
    enhance_mask_box_threshold: float = Field(
        0.30,
        description="The default enhance mask box threshold to use.",
        ge=0.0,
        le=1.0,
    )
    enhance_mask_sam_max_detections: int = Field(
        0,
        description="The default enhance mask sam max detections to use.",
        ge=0,
        le=10,
    )

    enhance_inpaint_disable_initial_latent: bool = False
    enhance_inpaint_engine: str = Field(
        "2.6", description="The default enhance inpaint engine version to use."
    )

    enhance_inpaint_strength: float = Field(
        1.0, description="The default enhance inpaint strength to use.", ge=0.0, le=1.0
    )
    enhance_inpaint_respective_field: float = Field(
        0.618,
        description="The default enhance inpaint respective field to use.",
        ge=0.0,
        le=1.0,
    )
    enhance_inpaint_erode_or_dilate: int = Field(
        0,
        description="The default enhance inpaint erode or dilate to use.",
        ge=-64,
        le=64,
    )
    enhance_mask_inver: bool = False

    enhance_uov_method: Optional[UPSCALE_OR_VARIATION_MODES] = Field(
        None, description="Enhacement uov method"
    )
    enhance_uov_method: Optional[str] = Field(
        None, description="The default enhance uov method to use."
    )
    enhance_uov_processing_order: int = Field(
        ENHANCEMENT_UOV_BEFORE,
        description="The default enhance uov processing order to use.",
    )
    enhance_uov_prompt_type: int = Field(
        ENHANCEMENT_UOV_PROMPT_TYPE_ORIGINAL,
        description="The default enhance uov prompt type to use.",
    )
...
class _InitialImageGenerationParams(BaseModel):
    ...
    image_number: int = Field(
        1, description="The default amount of images number to use.", ge=1
    )
    ...
    vae_downloads: dict[str, str] = Field(
        {}, description="The default vae downloads to use."
    )
    lora_downloads: dict[str, str] = Field(
    ...
    clip_skip: int = Field(
        2, description="The default clip skip to use."
    )  # Where clip skip max?
    ...
    describe_content_type: Optional[List[str]] = Field(
        [DESCRIBE_TYPE_PHOTO], description="The default describe content type to use."
    )

    use_image_prompt_advanced: bool = Field(
        False, description="Bool: should use image prompt advanced?"
    )
    ...
    uov_method: Optional[UPSCALE_OR_VARIATION_MODES] = Field(
        None, description="The default uov method to use."
    )
    steps_uov: int = -1
...
class ImageGenerationObject(_InitialImageGenerationParams):
    ...
    def download_models(self):
        ...
        for file_name, url in vae_approx_filenames:
            load_file_from_url(
                url=url,
                model_dir=FolderPathsConfig.path_vae_approx,
                file_name=file_name,
            )
        ...
        for file_name, url in self.checkpoint_downloads.items():
            model_dir = os.path.dirname(
                get_file_from_folder_list(file_name, FolderPathsConfig.path_checkpoints)
            )
            load_file_from_url(url=url, model_dir=model_dir, file_name=file_name)
        for file_name, url in self.embeddings_downloads.items():
            load_file_from_url(
                url=url,
                model_dir=FolderPathsConfig.path_embeddings,
                file_name=file_name,
            )
        for file_name, url in self.lora_downloads.items():
            load_file_from_url(
                ...
                model_dir=FolderPathsConfig.path_loras,
                file_name="sd_xl_offset_example-lora_1.0.safetensors",
            )
        for file_name, url in self.vae_downloads.items():
            load_file_from_url(
                url=url, model_dir=FolderPathsConfig.path_vae, file_name=file_name
            )
